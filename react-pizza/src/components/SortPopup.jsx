...
function SortPopup({ items, onClickItem }) {
  ...
  const activLable = items[activItem];
  ...
  const hendlerOutsideClick = (e) => {
    ...
    // setVisiblePopup(!!e.path.includes(sortRef.current));
  };
  ...
  return (
    // <div ref={(ref) => { sortRef.current = ref; }} className="sort"> длинная запись реф
    <div ref={sortRef} className="sort">
      <div className="sort__label">
        ...
        <b>Сортировка по:</b>
        <span onClick={toggelVisibllePopup}>{activLable}</span>
      </div>
      { visiblePopup && (
